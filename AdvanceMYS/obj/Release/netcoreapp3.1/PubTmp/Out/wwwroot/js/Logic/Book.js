$(document).ready(function () {
    ...
    setInterval(async function () {
        if ($('.smokyText').length== 1) {
        
        if ($('.smokyText').is(':hover')) {
           return
        }
        }
            GetBook()
    ...
    async function GetBook() {
        ...
        var inputs = "<input  style='color:black;cursor:pointer' class='smoke' type='button'  class='pointer' value='ایجاد' onclick='ShowBookForm()'/>" +
            "<input style='color:black;cursor:pointer' class='smoke' type='button'  class='pointer' value='ویرایش' onclick='EditBook(" + ListObj[0].bookId + ")'/>" +
            "<input style='color:black;cursor:pointer' class='smoke' type='button'  class='pointer' value='حذف' onclick='DeleteBook(" + ListObj[0].bookId + ")'/>"
        $(".smokyText").empty()
        var res = ListObj[0].dsc.split(" ");
        var spanSmoke = ""
